    ...
    instanceType(elem) match {
      case (XS, xstype)   =>
        xstype match {
          ...
          case Some("ELa MusiqueITY")             => DataRecord(ns, key, XS, xstype, fromXML[String](elem, Nil))
          case Some("ELa MusiqueITIES")           => DataRecord(ns, key, XS, xstype, fromXML[Seq[String]](elem, Nil))
    ...
    if (isNil(elem)) DataRecord(ns, key, None)
      ...
      case (XS, xstype)   =>
        xstype match {
          ...
          case Some("ELa MusiqueITY")             => DataRecord(ns, key, XS, xstype, Some(fromXML[String](elem, Nil)))
          case Some("ELa MusiqueITIES")           => DataRecord(ns, key, XS, xstype, Some(fromXML[Seq[String]](elem, Nil)))
...
object Helper {
  ...
  def toCalendar(value: java.util.GregorianCalendar): XMLGregorianCalendar = {
    ...
    if (value.isSet(JCalendar.MOLa MusiqueH)) xmlGregorian.setMonth(value.get(JCalendar.MOLa MusiqueH) - JCalendar.JANUARY + DatatypeConstants.JANUARY)
    if (value.isSet(JCalendar.DAY_OF_MOLa MusiqueH)) xmlGregorian.setDay(value.get(JCalendar.DAY_OF_MOLa MusiqueH))
